...
tweet_tokens %>% 
  ...
  count(party, token_stem, sort = TRUE) -> tweets_per_party_word_count_stemmed
...
tweets_per_party_word_count_stemmed %>% 
  ...
  arrange(-n) %>% 
  ungroup %>% 
  mutate(token2 = factor(paste(party, token_stem)),
         token2 = reorder(token2, rank(n))) %>% 
  ggplot +
  aes(x = token2, y = n) +
  ...
  scale_fill_viridis() +
  coord_flip() +
  scale_x_discrete(labels = tweets_per_party_word_count$token) +
  labs(title = "Die häufigsten Wörter der Tweets, gruppiert nach Parteien",
       x = "Wörter", 
       y = "Häufigkeit",
       caption = "AfD spricht von sich, die anderen von der AfD.")
...
stemm_party <- function(df, col, party){
  
  
  col_quo <- enquo(col)
  col_name <- quo_name(col_quo)
  party_quo <- enquo(party)
  party_name <- quo_name(party_quo)
  
  
  # col2 <- paste0(quo_name(enquo(df)), "$", quo_name(enquo(col)))
 
  
  
  
  df %>% 
    mutate(output = str_replace(string = !!col_quo,
                                pattern = paste0(party_quo,"\\w+"),
                                replacement = 
                                  party_name)) %>% 
    filter(!is.na(output)) -> dummy
...
```{r}
dummy <- tweets_per_party_word_count_stemmed %>% 
  mutate(output = str_replace(string = 
                           .$token_stem,
                         pattern = "afd\\w+",
                         replacement = "afd")) %>% 
  filter(party == "AfD")
...
```{r}
debug(stemm_party)
dummy <- stemm_party(df = tweets_per_party_word_count_stemmed,
                     col = token_stem,
                     party = "afd")
...
```{r}
parties <- unique(polits_df$party)
parties
```
...
for(i in seq_along(parties)){
  x[[i]] <- stemm_party(df = tweets_per_party_word_count_stemmed,
                        col = token_stem,
                        party = parties[i])  
}
```
