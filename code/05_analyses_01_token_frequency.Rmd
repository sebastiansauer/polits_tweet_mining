...
ggsave(p_tweets_timeline2, file = "p_tweets_timeline2.pdf")
```
...
polits_df %>% 
  group_by(party) %>% 
  filter(n > 1) %>% 
  summarise(tweets_avg = mean(daily_tweets_n)) %>% 
  na.omit %>% 
  ggplot +
  aes(x = reorder(party, -tweets_avg), y = tweets_avg) +
  geom_col(aes(fill = party)) +
  labs(x = "Partei",
       y = "Mittlere Anzahl an Tweets pro Tag") +
  scale_fill_manual(values = party_pal) -> p_tweets_day_party

p_tweets_day_party
# xxx
ggsave(p_tweets_day_party, file = "img/p_tweets_day_party.pdf")
```
