...
polits_df %>% 
  ...
  mutate(neg_words_n = trump_emo_words$neg_words_n,
         pos_words_n = trump_emo_words$pos_words_n,
         emo_words_n = trump_emo_words$emo_words_n,
         neg_words_ratio = trump_emo_words$neg_words_ratio) %>% 
  bind_rows(polits_df) -> polits_df
...
polits_df %>% 
  ...
  coord_flip() +
  scale_fill_gradient(low = "white", high = "red") -> p_neg_words_ratio
...
polits_df %>% 
  ...
  geom_point(aes(color = party, shape = `Emo-Kennzahl`)) +
  coord_flip() +
  scale_color_brewer(palette = "Set1") +
...
polits_df %>% 
  select(screenName, emo_score, emo_abs_score) %>% 
  arrange(-emo_abs_score) %>% 
  slice(1:25) %>% 
  ggplot +
...
party_emo_weighted %>% 
 # mutate(emo_score = `*`(emo_score, 1)) %>% 
  ...
  geom_line(aes(group = emo_type), color = "grey40") +
  geom_point(aes(color = party, shape = emo_type)) +
  coord_flip() +
  scale_color_brewer(palette = "Set1") +
  labs(x =  "Partei",
       y = "Emotionalität")
